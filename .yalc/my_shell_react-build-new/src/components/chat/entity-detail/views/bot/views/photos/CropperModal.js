...
function CropperModal({ type, botId, file, open, onClose }) {
    ...
    const handleUpLoadImg = (0, react_2.useCallback)(async () => {
        ...
        try {
            ...
            fetch(dataUrl)
                .then(res => res.blob())
                .then(blob => {
                const _file = new File([blob], `cropped-image.${ext}`, { type: getTypeFromExt(ext) });
                return (0, common_1.uploadFileToS3)(common_1.Scenario.SCENARIO_BOT_PHOTO, ext, _file);
            })
                .then(res => {
                return (0, bot_1.uploadBotPhoto)(botId, [res.objectKey], type);
            })
                .then(res => {
    ...
    }, [botId, cropper2Ref, ext, onClose, type, warning]);
