...
const useClearMemory_1 = __importDefault(require("../hooks/useClearMemory.js"));
function ClearMemory({ type, id, disabled = false, onSuccess }) {
    ...
    const { addMessage } = (0, react_1.useContext)(MessageContext_1.MessageContext);
    const chatLocale = (0, next_intl_1.useTranslations)('chat');
    const clearMemory = (0, useClearMemory_1.default)(type, id, addMessage, name);
    const onClick = async () => {
        try {
            clearMemory();
            onSuccess();
