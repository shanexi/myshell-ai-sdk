...
import useClearMemory from '../hooks/useClearMemory.js';
export default function ClearMemory({ type, id, disabled = false, onSuccess }) {
    const { entityInfo } = useContext(StaticContext);
    const { name } = entityInfo;
    const { addMessage } = useContext(MessageContext);
    const chatLocale = useTranslations('chat');
    const clearMemory = useClearMemory(type, id, addMessage, name);
    const onClick = async () => {
        try {
            clearMemory();
            onSuccess();
