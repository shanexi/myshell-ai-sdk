...
function GalleryList({ botId, className }) {
    ...
    const deleteCallback = (id) => {
        const galleryListFilter = galleryList?.filter(item => item.id !== id);
        setGalleryList(galleryListFilter);
    };
    const followCallback = (userId) => {
        const galleryListUpdate = galleryList?.map(item => {
        ...
        setGalleryList(galleryListUpdate);
    };
    return ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [loading && galleryList.length === 0 && (0, jsx_runtime_1.jsx)(GalleryListSkeleton_1.default, { isMobile: isMobile }), !fetchError && galleryList?.length > 0 ? ((0, jsx_runtime_1.jsx)(react_infinite_scroll_component_1.default, { className: (0, utils_1.cn)('w-full overflow-auto space-y-3 pb-24 md:pb-3 md:pt-[60px]', className), dataLength: galleryList.length, next: () => {
                    getGalleryData(nextPageToken);
                }, hasMore: hasMore, loader: "", endMessage: "", height: isMobile ? 'calc(100vh - 56px)' : 'calc(100vh - 16px)', children: (0, jsx_runtime_1.jsxs)(masonry_1.default, { className: "my-masonry-grid ml-2 md:ml-0", columnClassName: "my-masonry-grid_column", children: [galleryList?.map((item, i) => {
