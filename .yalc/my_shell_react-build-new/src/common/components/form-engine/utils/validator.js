...
function genStringFormat(schema, format, message) {
    if (format === 'url') {
        ...
        return schema.refine(arg => /^([\u0024\u00A2\u00A3\u00A4\u20AC\u00A5\u20B1\u20B9\uFFE5]\s*)(\d+,?)+(\.\d+)?\s*$/.test(arg), message);
    }
    else if (format === 'date') {
        return schema.refine(arg => {
            const date = zod_1.z.coerce.date();
            return date.safeParse(arg).success;
        }, message);
    }
    return schema;
}
function genStringValidatorRules(rules, schema) {
...
function genNumberValidatorRules(rules, schema) {
    return rules.reduce((prev, rule) => {
        const { message, max, min, maximum, minimum, exclusiveMaximum, exclusiveMinimum, multipleOf } = rule;
        if (max) {
...
function createValidator(schema) {
    ...
    const { required: requiredRule, message: requiredMessage } = rules?.find?.(item => item.required) || {};
    const { format: formatRule, message: formatMessage } = rules?.findLast?.(item => item.format) || {};
    const { const: constRule, message: constMessage } = rules?.findLast?.(item => item.const) || {};
    const { enum: enumRule, message: enumMessage } = rules?.findLast?.(item => item.enum) || {};
    let ret = null;
    if (type === 'string') {
        const source = genStringValidatorRules(rules, zod_1.z.string({ required_error: requiredMessage }));
        if (formatRule) {
        ...
        if (enumRule) {
            ret = ret?.refine(data => enumRule.includes(data), { message: enumMessage });
        }
        if (constRule) {
        ...
        ret = genNumberValidatorRules(rules, zod_1.z.number({ required_error: requiredMessage }));
        if (enumRule) {
            ret = ret?.refine(data => enumRule.includes(data), { message: enumMessage });
        }
        if (constRule) {
