...
exports.default = useWidgetRecorder;
...
const reducer = (state, action) => {
    ...
    switch (type) {
        ...
        case RecordActionType.SET_TIME: {
            ...
            return { ...state, duration: state.duration + 1, status: status, timeLeft: newTimeLeft };
        }
...
function useWidgetRecorder(widgetInfo) {
    ...
    }, [destroyAudioRecorder, error, state.recorder, supportedType]);
    const stopRecording = (0, react_1.useCallback)(async () => {
        return await destroyAudioRecorder();
    }, [destroyAudioRecorder]);
    (0, react_1.useEffect)(() => {
        if (state.recorder) {
    ...
    }, [changePlayingAudio, stopRecording, state.recorder, setInputType]);
    ...
    const { sendVoiceMessage } = (0, useWidgetVoiceMessageSender_1.default)({ audioType: supportedType, clearRecord, widgetInfo });
    (0, react_1.useEffect)(() => {
        const keydownHandler = async (e) => {
            ...
            if (e.code == 'Space') {
                ...
                if (state.status === RecordStateEnum.Stopped) {
                    await sendVoiceMessage({ recordState: state });
                }
            }
            if (e.code == 'Enter') {
                e.preventDefault();
    ...
    }, [clearRecord, state, sendVoiceMessage]);
    return {
        supportedType,
        startRecording,
        stopRecording,
