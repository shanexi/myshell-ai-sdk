...
const getErrorMsgByType = (requestErrorTranslations, type) => {
    switch (type) {
        case interfaces_1.MessageRunningErrorType.RUNNING_ERROR_TYPE_ENGINE_ERROR:
            return requestErrorTranslations('chat_error_tip');
        case interfaces_1.MessageRunningErrorType.RUNNING_ERROR_TYPE_INNER_COMPONENT_ERROR:
            return requestErrorTranslations('inner_component_error_tip');
        case interfaces_1.MessageRunningErrorType.RUNNING_ERROR_TYPE_LLM_TOKEN_TOO_LONG:
...
const RunningError = ({ chat }) => {
    const t = (0, next_intl_1.useTranslations)('chat');
    const errorT = (0, next_intl_1.useTranslations)('request.error');
    const [isOpen, setIsOpen] = (0, react_1.useState)(false);
    const setMessageHandled = (0, store_1.useChatStore)(state => state.setMessageHandled);
    const { onCopy } = (0, useCopyClipboard_1.default)(chat?.runningError?.errorDetail || '');
    ...
    const errorMessage = (0, react_1.useMemo)(() => (0, exports.getErrorMsgByType)(errorT, chat?.runningError?.errorType), [chat?.runningError?.errorType]);
    if (chat?.runningError?.errorType === interfaces_1.MessageRunningErrorType.RUNNING_ERROR_TYPE_ENERGY_INSUFFICIANT) {
        if (energy > (chat?.runningError?.requireEnergyToUse || 0)) {
            return null;
        }
