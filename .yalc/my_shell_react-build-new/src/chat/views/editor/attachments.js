...
const audio_1 = __importDefault(require("./audio.js"));
...
function File({ data, onDelete, onPreview, uploadFiles, index, isMobile, isChoosingFile }) {
    ...
    const handlePreview = (index) => {
        ...
        if (isError) {
            uploadFiles(data.botId, [{ ...data, status: 'pending' }], true);
        }
    };
...
function Attachments({ botId, onDelete, isMobile, isChoosingFile }) {
    ...
    const files = (0, store_1.useChatStore)(state => state.fileUpload.filesMap[botId]);
    ...
    const renderItems = (files) => {
        return (files || []).map((item, index) => {
            return ((0, jsx_runtime_1.jsx)(FileCard, { data: item, onDelete: onDelete, onPreview: onPreview, uploadFiles: uploadFiles, index: index, isMobile: isMobile, isChoosingFile: isChoosingFile }, item.id));
        });
    };
    if ((files || []).length === 0) {
        return null;
...
exports.default = (0, react_1.memo)(Attachments);
