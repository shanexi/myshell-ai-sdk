...
// 读取 styles.css 文件
const stylesPath = path.join(__dirname, '../apps/bot-list/src/styles.css');
const content = fs.readFileSync(stylesPath, 'utf8');

// 找到相关的区域
const baseToDesignTokenRegex = /\/\* BASE \*\/([\s\S]*?)\/\* DESIGN TOKEN \*\//;
const designTokenToGradientRegex = /\/\* DESIGN TOKEN \*\/([\s\S]*?)\/\* _GRADIENT PALETTE \*\//;
...
if (baseMatch) {
  ...
  const updatedBaseSection = baseSection.replace(/--redius-radius-/g, '--radius-');
  newContent = newContent.replace(baseToDesignTokenRegex, `/* BASE */${updatedBaseSection}/* DESIGN TOKEN */`);
}
...
if (designTokenMatch) {
  ...
  // 替换原文件中的内容
  newContent = newContent.replace(designTokenToGradientRegex, `/* DESIGN TOKEN */${updatedSection}/* _GRADIENT PALETTE */`);
